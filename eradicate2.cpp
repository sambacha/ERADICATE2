...
bool printResult(const cl_int err) {
  std::cout << ((err != CL_SUCCESS) ? lexical_cast::write(err) : "OK")
            << std::endl;
...
std::string
makePreprocessorInitHashExpression(const std::string &strAddressBinary,
                                   const std::string &strInitCodeDigest) {
  std::random_device rd;
  std::mt19937_64 eng(rd());
  std::uniform_int_distribution<unsigned int>
      distr; // C++ requires integer type: "C2338	note : char, signed
             // char, unsigned char, int8_t, and uint8_t are not allowed"
  ethhash h = {0};
...
int main(int argc, char **argv) {
  try {
    ...
    size_t worksizeMax =
        0; // Will be automatically determined later if not overriden by user
    ...
    if (strInitCodeFile != "") {
      ...
      if (!ifs.is_open()) {
        std::cout << "error: failed to open input file for init code"
                  << std::endl;
        return 1;
      }
      strInitCode.assign(std::istreambuf_iterator<char>(ifs),
    ...
    const std::string strPreprocessorInitStructure =
        makePreprocessorInitHashExpression(strAddressBinary, strInitCodeDigest);
    ...
    for (size_t i = 0; i < vFoundDevices.size(); ++i) {
      ...
      std::cout << "  GPU" << i << ": " << strName << ", " << globalMemSize
                << " bytes available, " << computeUnits << " compute units"
                << std::endl;
    ...
    const std::string strBuildOptions =
        "-D ERADICATE2_MAX_SCORE=" + lexical_cast::write(ERADICATE2_MAX_SCORE) +
        " -D ERADICATE2_INITHASH=" + strPreprocessorInitStructure;
    if (printResult(clBuildProgram(clProgram, vDevices.size(), vDevices.data(),
      ...
      size_t sizeLog;
      clGetProgramBuildInfo(clProgram, vDevices[0], CL_PROGRAM_BUILD_LOG, 0,
                            NULL, &sizeLog);
      char *const szLog = new char[sizeLog];
      clGetProgramBuildInfo(clProgram, vDevices[0], CL_PROGRAM_BUILD_LOG,
                            sizeLog, szLog, NULL);
    ...
    Dispatcher d(clContext, clProgram, worksizeMax == 0 ? size : worksizeMax,
                 size);
    for (auto &i : vDevices) {
