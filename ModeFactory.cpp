...
mode ModeFactory::zeros() { return range(0, 0); }
...
mode ModeFactory::matching(const std::string strHex) {
  ...
  for (size_t i = 0; i < strHex.size(); i += 2) {
    ...
    const auto indexLo = i + 1 < strHex.size()
                             ? hexValueNoException(strHex[i + 1])
                             : std::string::npos;
...
mode ModeFactory::letters() { return range(10, 15); }

mode ModeFactory::numbers() { return range(0, 9); }
