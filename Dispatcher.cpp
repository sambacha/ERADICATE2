...
static void printResult(
    const result r, const cl_uchar score,
  ...
  std::cout << strVT100ClearLine << "  Time: " << std::setw(5) << seconds
            << "s Score: " << std::setw(2) << (int)score << " Salt: 0x"
            << strSalt << " Address: 0x" << strPublic << std::endl;
}
...
Dispatcher::Device::Device(Dispatcher &parent, cl_context &clContext,
    ...
    : m_parent(parent), m_index(index), m_clDeviceId(clDeviceId),
      ...
      m_memResult(clContext, m_clQueue, CL_MEM_READ_WRITE,
                  ERADICATE2_MAX_SCORE + 1),
      m_memMode(clContext, m_clQueue, CL_MEM_READ_ONLY | CL_MEM_HOST_WRITE_ONLY,
                1),
      m_round(0) {}
...
Dispatcher::Dispatcher(cl_context &clContext, cl_program &clProgram,
                       const size_t worksizeMax, const size_t size)
    : m_clContext(clContext), m_clProgram(clProgram),
      m_worksizeMax(worksizeMax), m_size(size), m_clScoreMax(0),
...
void Dispatcher::run(const mode &mode) {
  ...
  for (auto it = m_vDevices.begin(); it != m_vDevices.end(); ++it) {
    ...
    *d.m_memMode = mode;
    ...
    CLMemory<cl_uchar>::setKernelArg(
        d.m_kernelIterate, 2, d.m_clScoreMax); // Updated in handleResult()
...
void Dispatcher::deviceDispatch(Device &d) {
  ...
  if (m_quit) {
    ...
    CLMemory<cl_uint>::setKernelArg(
        d.m_kernelIterate, 4,
        ++d.m_round); // Round information updated in deviceDispatch()
...
void CL_CALLBACK Dispatcher::staticCallback(cl_event event,
  ...
  if (event_command_exec_status != CL_COMPLETE) {
    throw std::runtime_error("Dispatcher::onEvent - Got bad status" +
                             lexical_cast::write(event_command_exec_status));
  }
